...
export function ChatPanel({
  ...
  if (isMinimized) {
    return (
      <motion.button
        ...
        className={`fixed z-20 w-12 h-12 rounded-full flex items-center justify-center ${isMobile ? "bottom-4 right-4" : "top-[84px] right-6"}`}
        style={{
  ...
  if (!isExpanded) {
    ...
    return (
      <motion.div
        ...
        className={`fixed z-20 ${isMobile ? "bottom-3 left-3 right-3" : "top-[84px] right-6"}`}
  ...
  return (
    <motion.section
      ...
      transition={{ type: "spring", stiffness: 300, damping: 25 }}
      className={`fixed z-20 flex flex-col liquid-glass-panel rounded-[1.5rem] overflow-hidden ${isMobile ? "bottom-3 left-3 right-3" : "top-[84px] right-6"}`}
