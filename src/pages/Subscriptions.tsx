...
import { useMapMarkers } from "@/hooks/useMapData";

export default function Subscriptions() {
  const navigate = useNavigate();
  const { isLoggedIn, hasCompletedOnboarding } = useAuthStore();
  const { data: subscribedIds, isLoading: subsLoading } = useSubscriptions();
  const { data: markersData } = useMapMarkers("zukunft");
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <Button
                    ...
                    onClick={() => handleUnsubscribe(marker.id)}
